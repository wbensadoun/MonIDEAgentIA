...
const AIChat = ({
  ...
  const swapAboveInput = (
    <>
      ...
      {explicitContext.length > 0 && <div className="ai-message-file-refs">{explicitContext.map((filePath) => { const fileName = filePath.split(/[\\/]/).pop(); return <span key={filePath} className="ai-message-file-ref">@{fileName}<button type="button" className="ai-message-file-ref-close" aria-label={`Retirer ${fileName} du contexte`} onClick={() => removeExplicitContext(filePath)}>×</button></span>; })}</div>}
